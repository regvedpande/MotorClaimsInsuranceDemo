...
export default function CashlessAuths() {
  ...
  return (
    <>
      ...
      <Dialog open={Boolean(claim)} onClose={() => setClaim(null)} fullWidth maxWidth="md"><DialogTitle>Cashless Authorisation Letter</DialogTitle><DialogContent>{claim && <><Typography variant="h5">{claim.authCode}</Typography><Typography>Claim: {claim.claimNumber} | Customer: {claim.customerName}</Typography><Typography sx={{ mt: 2 }}>Approved line items: {claim.lineItems.filter((i) => i.status === 'Approved').map((i) => i.description).join(', ')}</Typography><Typography variant="h6" sx={{ mt: 2 }}>Insurer shall pay: {formatCurrency(claim.netPayable)}</Typography><Typography>Customer to pay at delivery: {formatCurrency(claim.customerPays)} (excess + inadmissibles)</Typography><Typography>Valid till: 30-Apr-2024</Typography></>}</DialogContent></Dialog>
    </>
  );
}

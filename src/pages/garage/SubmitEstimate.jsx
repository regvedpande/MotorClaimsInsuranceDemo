...
export default function SubmitEstimate() {
  const { id } = useParams();
  const { claims, notify, updateClaim, updateClaimStatus } = useApp();
  ...
  const submitEstimate = () => {
    const lineItems = rows.map((row, index) => ({
      id: `EST-${index + 1}`,
      description: row.description || `Estimate line ${index + 1}`,
      category: row.category,
      billed: Number(row.labour || 0) + Number(row.parts || 0),
      admissible: Number(row.labour || 0) + Number(row.parts || 0),
      status: 'PendingReview'
    }));
    updateClaim(claim.claimNumber, {
      lineItems,
      totalBill: totals.labour + totals.parts,
      estimatedLoss: totals.labour + totals.parts
    });
    updateClaimStatus(claim.claimNumber, 'UnderAssessment', 'Garage Partner', 'GaragePortal');
    notify('Estimate submitted to surveyor');
  };
  return (
    <>

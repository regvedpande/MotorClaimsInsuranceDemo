...
export default function ActiveSurvey() {
  ...
  const setStatus = (id, status) => setItems((current) => current.map((item) => {
    if (item.id !== id) return item;
    if (status === 'Reduced') {
      return { ...item, status, admissible: Math.round(item.billed * 0.7) };
    }
    return { ...item, status, admissible: status === 'Rejected' ? 0 : item.billed };
  }));
  return (
    <>
